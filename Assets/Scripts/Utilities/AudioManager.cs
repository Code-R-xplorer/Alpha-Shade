    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        public void Play(string name, Transform parent)
        {
            ...
            if (AudioItems.Exists(item => item.Name == name))
            {
                ...
                Transform oldSource = parent.Find($"{item.Name} (Temp)");
                
                if (oldSource != null)
                {
                    GameObject o = oldSource.gameObject;
        ...
        public void PlayRandom(string[] names, Transform parent)
        {
            int index = Random.Range(0, names.Length);
            Play(names[index], parent);
        }
